...
FEEDBACK_LEARNING_XLSX_PATH = DATA_DIR / "decline_feedback_learning.xlsx"
...
def _workbook_headers(worksheet: Any) -> list[str]:
    return [
        _safe_excel_text(cell.value)
        for cell in next(worksheet.iter_rows(min_row=1, max_row=1), [])
    ]


def _append_feedback_learning_rows(payload: dict[str, Any]) -> dict[str, Any]:
    try:
        from openpyxl import Workbook, load_workbook
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError("openpyxl is required to save feedback learning rows.") from exc

    feedback_items = payload.get("feedback_items")
    if not isinstance(feedback_items, list) or not feedback_items:
        raise ValueError("feedback_items must contain at least one valid feedback row.")
    ...
    if FEEDBACK_LEARNING_XLSX_PATH.exists():
        workbook = load_workbook(FEEDBACK_LEARNING_XLSX_PATH)
        worksheet = None
        for candidate in workbook.worksheets:
            existing_headers = _workbook_headers(candidate)
        ...
        if worksheet is None:
            old_rows = []
            for candidate in workbook.worksheets:
                old_headers = _workbook_headers(candidate)
                for row in candidate.iter_rows(min_row=2, values_only=True):
                    row_map = dict(zip(old_headers, row))
                    attribute_name = _safe_excel_text(
                        row_map.get("Business Attribute") or row_map.get("Attribute Name")
                    )
                    attribute_description = _safe_excel_text(
                        row_map.get("Business Definition") or row_map.get("Attribute Description")
                    )
                    if attribute_name and attribute_description:
                        old_rows.append(
                            [
                                attribute_name,
                                attribute_description,
                                _safe_excel_text(row_map.get("Region")),
                                _safe_excel_text(row_map.get("Value Stream") or row_map.get("Selected Value Stream")),
                                _safe_excel_text(row_map.get("Brief Problem Statement / Use Case") or row_map.get("Brief Problem Statement")),
                                _safe_excel_text(row_map.get("System Requirements")),
                                _safe_excel_text(row_map.get("Captured At")),
                                _safe_excel_text(row_map.get("Source File Name")),
                                _safe_excel_text(row_map.get("Original Feedback")),
                            ]
                        )

            workbook = Workbook()
            worksheet = workbook.active
            worksheet.title = "Requirement_Feedback"
            worksheet.append(headers)
            for old_row in old_rows:
                worksheet.append(old_row)
    else:
        workbook = Workbook()
        worksheet = workbook.active
        worksheet.title = "Requirement_Feedback"
    ...
    source_file_name = _safe_excel_text(payload.get("source_file_name"))
    feedback_text = _safe_excel_text(payload.get("feedback_text"))
    ...
    for item in feedback_items:
        ...
        attribute_name = _safe_excel_text(item.get("business_attribute") or item.get("attribute_name"))
        attribute_description = _safe_excel_text(item.get("business_definition") or item.get("attribute_description"))
        if not attribute_name or not attribute_description:
            continue
        ...
        worksheet.append(
            [
                attribute_name,
                attribute_description,
                _safe_excel_text(item.get("region")),
                _safe_excel_text(item.get("value_stream") or item.get("selected_value_stream")),
                _safe_excel_text(item.get("brief_problem_statement")),
                _safe_excel_text(item.get("system_requirements")),
                captured_at,
                source_file_name,
                feedback_text,
            ]
    ...
    if not saved_rows:
        raise ValueError("No valid attribute feedback rows were found to save.")

    workbook.save(FEEDBACK_LEARNING_XLSX_PATH)
    return {
        "status": "saved",
        "saved_rows": saved_rows,
        "saved_path": str(FEEDBACK_LEARNING_XLSX_PATH),
    }


def _send_sme_email(payload: dict[str, Any]) -> dict[str, Any]:
    smtp_host = _safe_excel_text(os.getenv("SMTP_HOST"))
    smtp_port = int(os.getenv("SMTP_PORT", "587"))
    smtp_username = _safe_excel_text(os.getenv("SMTP_USERNAME"))
    smtp_password = _safe_excel_text(os.getenv("SMTP_PASSWORD"))
    smtp_from = _safe_excel_text(os.getenv("SMTP_FROM") or smtp_username)
    smtp_to = _safe_excel_text(os.getenv("SME_EMAIL_TO"))
    subject = _safe_excel_text(payload.get("subject"))
    body = _safe_excel_text(payload.get("body"))
    ...
    if missing:
        raise ValueError(
            ...
            + ", ".join(missing)
            + ". For Gmail, use SMTP_HOST=smtp.gmail.com, SMTP_PORT=587, SMTP_USERNAME, and a Gmail app password in SMTP_PASSWORD."
        )
...
@app.post("/analytics/feedback/learning")
async def analytics_feedback_learning_endpoint(request: Request) -> dict[str, Any]:
    try:
        payload = await request.json()
        if not isinstance(payload, dict):
            raise ValueError("Feedback payload must be a JSON object.")
        return _append_feedback_learning_rows(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        logging.exception("Feedback learning save failed")
        raise HTTPException(
            status_code=500,
            detail="Feedback learning file save failed. Verify DATA folder access and openpyxl dependency.",
        ) from exc


@app.post("/analytics/sme-email")
async def analytics_sme_email_endpoint(request: Request) -> dict[str, Any]:
    try:
        payload = await request.json()
        if not isinstance(payload, dict):
            raise ValueError("Email payload must be a JSON object.")
        return _send_sme_email(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        logging.exception("SME email send failed")
        raise HTTPException(
            status_code=502,
            detail="SME email send failed. Verify SMTP configuration and network access.",
        ) from exc
